...
def virtual_machines(request, course_id):
    ...
    for vm in virtual_machines:
        ...
        #logger.debug("-++++++_____+++++------+++_____" + str(len(user_vm_configs)))
...
def start_vm(request, course_id, vm_id):
    ...
    """
    logger.debug("starting vm - " + str(request.user.id)+'_'+ str(course_id) +'_'+ str(vm_id))
    started_vm = None
    vm = None
    try:
        ...
        logger.error(str(e))
        audit(request, 'Error starting Virtual machine ' + str(vm.name) + '( ' + str(e) + ' )')
        if 'Connection refused' not in str(e).rstrip() or started_vm is not None:
            XenClient().remove_network_bridges(config.xen_server, request.user, course_id, vm_id)
...
def stop_vm(request, course_id, vm_id):
    ...
    """
    logger.debug("stopping vm - " + str(request.user.id) + '_' + str(course_id) + '_' + str(vm_id))
...
def rebase_vm(request, course_id, vm_id):
    ...
    """
    logger.debug("rebasing vm - " + str(request.user.id) + '_' + str(course_id) + '_' + str(vm_id))
    virtual_machine = Virtual_Machine.objects.get(pk=vm_id)
