        ...
        if form.is_valid():
            ...
            try:
                ...
                if not user.is_active:
                    #logger.info("Activation Code is: "+user.activation_code)
                    if user.activation_code is None or user.activation_code == "":
                        logger.info("Activation Code regenerated for: "+user.email)
                        activation_code = str(uuid.uuid4()).replace('-', '')
                        user.activation_code = activation_code
                        user.save()
                    else:
                        logger.info("Activation Code exists for: "+user.email)
                    ...
                    return render(request, 'vital/user_registration_validate.html', {'message': 'Activation Code sent to user again '
